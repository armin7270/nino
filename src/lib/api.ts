...
export const api = {
  ...
  async getPublicEndpoint(): Promise<{ publicEndpoint: PublicEndpoint }> {
    return apiRequest<{ publicEndpoint: PublicEndpoint }>('/api/public-endpoint');
  },

  /** Promote a configuration to own the single shared external AnyTLS port. */
  async activateConfig(id: string): Promise<{ success: boolean; gatewayConfigId?: string }> {
    return apiRequest<{ success: boolean; gatewayConfigId?: string }>(
      `/api/configs/${id}/activate`,
      { method: 'POST' }
    );
  },

  /** (Re)download the anytls-server binary inside the container. */
  async installBinary(): Promise<{
    success: boolean;
    ...
    return apiRequest<{
      ...
      binaryPath: string | null;
      binaryDownloadError?: string | null;
    }>('/api/maintenance/install-binary', { method: 'POST' });
  },
