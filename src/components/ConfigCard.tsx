...
interface ConfigCardProps {
  ...
  onActivateGateway?: (id: string) => void;
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {/* Public Gateway Badge */}
              {config.isGateway && isActive && (
                <span
                  title="This configuration owns the shared public AnyTLS port"
                  className="inline-flex items-center gap-1 rounded border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-[10px] font-mono text-amber-400"
                >
            ...
            <p className="text-xs text-white/40 font-mono mt-0.5">
              {config.isGateway ? 'Public' : 'Private'} {getPublicPort(config)}
              {config.listenPort ? (
                <span className="text-white/25"> → 127.0.0.1:{config.listenPort}</span>
              ) : null}{' '}
              | SNI:{' '}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-white/5 bg-white/[0.01] p-3 sm:px-4">
        ...
        <div className="flex items-center gap-2">
          {/* Promote this config to the shared public port (single TCP proxy). */}
          {isActive && !config.isGateway && onActivateGateway && (
            <button
              onClick={() => onActivateGateway(config.id)}
              title="Bind this configuration to the shared public AnyTLS port"
              className="flex items-center gap-1.5 rounded-lg border border-amber-500/20 bg-amber-500/5 px-2.5 py-1.5 text-xs font-medium text-amber-400 transition hover:bg-amber-500/15"
            >
          ...
          <button
            ...
            title={
              config.isGateway
                ? 'Copy direct AnyTLS connection link'
                : 'Copy link (activate this config to serve it on the public port)'
            }
