...
import { AnyTlsConfig, ServerStatus, RenewOptions, PublicEndpoint } from './types';
...
export default function App() {
  ...
  const [gatewayConfigId, setGatewayConfigId] = useState<string | null>(null);
  const [onRailway, setOnRailway] = useState<boolean>(false);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (configsRes.publicEndpoint) setPublicEndpoint(configsRes.publicEndpoint);
      if (configsRes.gatewayConfigId !== undefined) {
        setGatewayConfigId(configsRes.gatewayConfigId ?? null);
      }
      if (statusRes) {
        ...
        if (statusRes.onRailway) {
          setOnRailway(true);
        }
        if (statusRes.publicEndpoint) setPublicEndpoint(statusRes.publicEndpoint);
      }
    } catch (err: any) {
  ...
  // Auto-refresh every 30 seconds while logged in so expiry and traffic
  // changes are reflected without a manual page reload.
  ...
  // Promote a config to own the single shared external AnyTLS port (Railway
  // allows only one TCP proxy per service, so only one tunnel can be exposed).
  const handleActivateGateway = async (id: string) => {
    try {
      await api.activateConfig(id);
      setGatewayConfigId(id);
      await fetchData();
      showToast('Configuration is now the active public AnyTLS endpoint');
    } catch (err: any) {
      showToast(err.message || 'Failed to activate configuration');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#090909] text-[#e0e0e0] pb-16">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 space-y-6">
        {/* Public endpoint banner (Railway) */}
        {onRailway && (
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  <h2 className="font-medium text-white text-base flex items-center gap-2">
                    ...
                    <span
                      className={`rounded-full border px-2 py-0.5 text-[11px] font-mono ${
                        publicEndpoint?.source === 'railway-tcp'
                          ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'
                          : 'bg-amber-500/10 border-amber-500/20 text-amber-400'
                      }`}
                    >
                      {publicEndpoint?.source === 'railway-tcp' ? 'TCP PROXY READY' : 'TCP PROXY MISSING'}
                    </span>
                  </h2>
                    ...
                    {publicEndpoint?.source === 'railway-tcp' ? (
                      <>
                        ...
                        <span className="font-mono text-amber-400">
                          {publicEndpoint.host}:{publicEndpoint.port}
                        </span>
                      </>
                    ) : (
                      <>
                        Create a Railway <strong className="text-white/70">TCP Proxy</strong> pointing at
                        internal port{' '}
                        <span className="font-mono text-amber-400">
                          {publicEndpoint?.gatewayPort || 8443}
                        </span>{' '}
              ...
              <div className="flex items-center gap-2.5 w-full sm:w-auto">
                <a
                  ...
                  className="flex flex-1 sm:flex-none items-center justify-center gap-1.5 rounded-xl border border-white/5 bg-white/5 px-3 py-2 text-xs sm:text-sm font-medium text-white/80 transition hover:bg-white/10 hover:text-white"
                >
      ...
      <ChangePasswordModal
        ...
        publicEndpoint={publicEndpoint || serverStatus?.publicEndpoint || null}
        isStandalone={isStandalone}
        onRailway={onRailway}
